...
const createAuthor = async function (req, res) {
	try {
	  ...
	  if (!validation.isValidString(fname)) {
		res.status(400).send({ status: false, msg: "fname must contain letters" });
		return;
	  }
	  ...
	  if (!validation.isValidString(lname)) {
		res.status(400).send({ status: false, msg: "lname must contain letters only" });
		return;
	  }
	  ...
	  if (!validation.isValidTitle(title)) {
		res.status(400).send({ status: false, msg: "Title must contain only one of these-'Mr','Mrs','Miss'" });
		return
	  }
	  ...
	  //5.validation on password
	 	if(!password){
	  	...
	  	if(password.length>19||password.length<8){
			res.status(400).send({status:false, mesg:"password must be in between 8 to 19 characters"})
			return
		}
	  ...
	  res.status(201).send({ status: true, mag:"Data created successfully", data: newAuthor });
	} catch (err) {
